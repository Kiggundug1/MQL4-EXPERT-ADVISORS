...
extern double dds_delay = 62000.0;
...
extern double Hscan = 15.0;
extern double magicn = 16384.0;
extern int ATRS = 1;
extern int ATR_PERIOD = 8;
extern string nameEA = "IVYBOT_USD_CHF";
double g_ima_336;
double g_ima_344;
double g_iadx_480;
double g_iadx_488;
...
int start() {
   ...
   if (g_str_len_600 > gd_604 || g_str_len_600 < gd_604) {
   ...
   g_maxlot_644 = MarketInfo("USDCHF", MODE_MAXLOT);
   g_minlot_652 = MarketInfo("USDCHF", MODE_MINLOT);
      ...
      if (OrderSymbol() == Symbol() && OrderType() <= OP_SELL && OrderComment() == nameEA) {
         ...
         if (OrderType() == OP_BUY) {
            if (TimeCurrent() - OrderOpenTime() > dds_delay) {
               ...
               prtAlert("IVYBOT_USD_CHF: Closing BUY order");
            }
            if (trailingStop > 0.0) {
            ...
            if (TimeCurrent() - OrderOpenTime() > dds_delay) {
               ...
               prtAlert("IVYBOT_USD_CHF: Closing SELL order");
            }
            if (trailingStop > 0.0) {
   ...
   if (l_count_4 < 1) {
      ...
      if (g_bool_576 && !g_bool_580) {
         if (risk_management == TRUE) gd_544 = Ask - gd_620 * Point;
         else
         ...
         Print("Lots (", lots, ")");
         Print("ATR_PLUG (", gd_620, ")");
         Print("riskperpip (", gd_636, ")");
         Print("dollarrisk (", gd_628, ")");
         ...
         if (g_ticket_592 < 0) {
            ...
            prtAlert("IVYBOT_USD_CHF: Buying");
         } else {
            ...
            if (risk_management == TRUE) {
               if (NormalizeDouble(OrderStopLoss(), gi_596) != NormalizeDouble(Ask - gd_620 * Point, gi_596)) OrderModify(g_ticket_592, OrderOpenPrice(), Ask - gd_620 * Point, Ask + takeProfit * Point, 0, Blue);
            } else {
               if (stopLoss > 0.0)
      ...
      if (g_bool_580 && !g_bool_576) {
         if (risk_management == TRUE) gd_544 = Bid + gd_620 * Point;
         else
         ...
         if (g_ticket_592 < 0) {
            Print("OrderSend (", nameEA, ") failed with error #", GetLastError());
            prtAlert("IVYBOT_USD_CHF: Selling");
         } else {
            ...
            if (risk_management == TRUE) {
               if (NormalizeDouble(OrderStopLoss(), gi_596) != NormalizeDouble(Bid + gd_620 * Point, gi_596)) OrderModify(g_ticket_592, OrderOpenPrice(), Bid + gd_620 * Point, Bid - takeProfit * Point, 0, Blue);
            } else {
               if (stopLoss > 0.0)
...
void calculateIndicators() {
   g_iatr_612 = iATR(NULL, 0, ATR_PERIOD, 1);
   gd_620 = NormalizeDouble(g_iatr_612 * ATRS, gi_596);
   ...
   g_bool_580 = gd_552 < (-1.0 * Dscan) && gd_560 < (-1.0 * Hscan) && g_ima_568 < Bid && DayOfWeek() < 5;
   gi_unused_584 = FALSE;
}
...
double calculatelots() {
   gd_620 *= gd_660;
   gd_628 = equitypercent * AccountEquity();
   gd_636 = gd_628 / gd_620;
   lots = gd_636 / gd_668;
   gd_676 = gd_636 / gd_668;
   if (lots < 10.0) {
      ...
      if (lots > g_maxlot_644) lots = g_maxlot_644;
      if (lots < g_minlot_652) lots = g_minlot_652;
   }
   return (lots);
}
