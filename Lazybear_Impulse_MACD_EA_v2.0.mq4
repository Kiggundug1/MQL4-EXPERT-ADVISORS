...
input bool   histogram_filter        = true;                    // Histogram Filter:
input bool   close_buys_after_orange = true;                    // Close Buys after Orange Candle (>0):
input bool   close_sells_after_green = true;                    // Close Sells after Green Candle (<0):
...
class BUYcondition2 : public iConditions
{
  public:
    bool evaluate()
    {
        ...
        double gray = Macd(grayLine, 1);
        double red  = Macd(redLine, 1);
        return (gray >= 0 && gray != EMPTY_VALUE) && (red >= 0 && red != EMPTY_VALUE);
    }
};
...
class ConditionOrangeBar : public iConditions
{
  public:
    bool evaluate()
    {
        return (Macd(orange, 1) > 0 && Macd(orange, 1) != EMPTY_VALUE) && (Macd(orange, 2) <= 0 || Macd(orange, 2) == EMPTY_VALUE);
    }
};
ConditionOrangeBar conditionOrangeBar;
...
class ConditionGreenBar : public iConditions
{
  public:
    bool evaluate()
    {
        bool r = (Macd(green, 1) < 0 && Macd(green, 1) != EMPTY_VALUE) && (Macd(green, 2) >= 0 || Macd(green, 2) == EMPTY_VALUE);
        Print(__FUNCTION__, " r: ", r);
        return r;
    }
};
ConditionGreenBar conditionGreenBar;
...
int                    OnInit()
{
    ...
    if(histogram_filter) conditionsToBuy.AddCondition(&buyCondition2);

    //--- sell:
    conditionsToSell.AddCondition(sellCondition1 = new SELLcondition1());
    if(histogram_filter) conditionsToSell.AddCondition(&sellCondition2);
    ...
    if (close_buys_after_orange) {
        conditionsToCloseBuy.AddCondition(&conditionOrangeBar);
    }
    if (close_sells_after_green) {
        conditionsToCloseSell.AddCondition(&conditionGreenBar);
    }
