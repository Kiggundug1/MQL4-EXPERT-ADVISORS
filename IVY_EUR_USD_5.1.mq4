...
extern double Pin_under = 28.0;
extern double Long_cut = 6.0;
extern double crmRPM = 14.0;
extern double Pound_level = 20.0;
extern double Dscan = 20.0;
extern double Hscan = 5.0;
extern double magicn = 16384.0;
extern int ATRS = 1;
extern double ATR_PERIOD = 14.0;
...
int start() {
   ...
   g_maxlot_640 = MarketInfo("EURUSD", MODE_MAXLOT);
   g_minlot_648 = MarketInfo("EURUSD", MODE_MINLOT);
   ...
   if (l_count_4 < 1) {
      ...
      if (g_bool_576 && !g_bool_580) {
         if (risk_management == TRUE) gd_544 = Ask - gd_616 * Point;
         else
         ...
         if (g_ticket_592 < 0) {
            ...
            if (risk_management == TRUE) {
               if (NormalizeDouble(OrderStopLoss(), gi_596) != NormalizeDouble(Ask - gd_616 * Point, gi_596)) OrderModify(g_ticket_592, OrderOpenPrice(), Ask - gd_616 * Point, Ask + takeProfit * Point, 0, Blue);
            } else {
               if (stopLoss > 0.0)
      ...
      if (g_bool_580 && !g_bool_576) {
         if (risk_management == TRUE) gd_544 = Bid + gd_616 * Point;
         else
         ...
         if (g_ticket_592 < 0) {
            ...
            if (risk_management == TRUE) {
               if (NormalizeDouble(OrderStopLoss(), gi_596) != NormalizeDouble(Bid + gd_616 * Point, gi_596)) OrderModify(g_ticket_592, OrderOpenPrice(), Bid + gd_616 * Point, Bid - takeProfit * Point, 0, Blue);
            } else {
               if (stopLoss > 0.0)
...
void calculateIndicators() {
   ...
   gd_616 = NormalizeDouble(g_iatr_608 * ATRS, gi_596);
