...
int start()
{
   ...
   for(i = 0; i<limit; i++) {
      if(ExtMapBuffer1[i] != EMPTY_VALUE) {
         ...
         entry = priceround(High[i] + (entry_from_hl+spread_inticks)*ticksize);
         stoploss = priceround(ExtMapBuffer1[i]-entry_from_hl*ticksize);
         ...
         entry = priceround(Low[i] - entry_from_hl*ticksize);
         stoploss = priceround(ExtMapBuffer2[i] + (sl_from_hl+spread_inticks)*ticksize);
...
int signal(int bar)
{
   ...
   if (signal != SIGNAL_NO) {
      ...
      if (signal == SIGNAL_BUY) {
         entry = priceround(high+(entry_from_hl+spread_inticks)*ticksize);
         stoploss = priceround((low-(iATR(Symbol(),0,atrforarrows,bar))*arrow_dist)-entry_from_hl*ticksize);
         takeprofit = priceround(entry+(entry-stoploss)*tp_from_sl);
      }
      if (signal == SIGNAL_SELL) {
         entry = priceround(low-entry_from_hl*ticksize);
         stoploss = priceround((high+(iATR(Symbol(),0,atrforarrows,bar))*arrow_dist)+(sl_from_hl+spread_inticks)*ticksize);
